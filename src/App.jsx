...
import {
  ...
  Facebook,
  Phone,
  Briefcase, // Added Briefcase icon for vendors
} from "lucide-react";
...
const Button = ({
  ...
  const baseStyle =
    "inline-flex items-center justify-center px-6 py-3 rounded-full font-medium transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2";
  const variants = {
    primary:
      "bg-gold-500 hover:bg-gold-600 text-white focus:ring-gold-500 shadow-lg shadow-gold-500/20",
    secondary:
      "bg-[#2D2424] text-white border border-gold-500/30 hover:bg-gray-800 focus:ring-gray-500", // Softened black to warm charcoal
    outline:
      "bg-transparent border-2 border-[#2D2424] text-[#2D2424] hover:bg-[#2D2424] hover:text-white",
    white: "bg-white text-[#2D2424] hover:bg-[#FFE4E6] shadow-lg", // Hover changed to bold blush
    goldOutline:
      "bg-transparent border-2 border-gold-500 text-gold-500 hover:bg-gold-500 hover:text-white", // New variant for dark backgrounds
  };
...
const PackageCard = ({
  ...
  >
    ...
    <div className="mb-6">
      <h3
        className={`text-2xl font-serif font-bold mb-2 ${recommended ? "text-gold-400" : "text-[#2D2424]"}`}
      >
        {title}
      </h3>
      <p
        className={`text-sm mb-4 h-10 ${recommended ? "text-gray-300" : "text-gray-500"}`}
      >
        {description}
      </p>
      <div className="flex items-baseline">
        ...
        {price !== "Custom" && (
          <span
            className={`text-sm ml-1 ${recommended ? "text-gray-400" : "text-gray-500"}`}
          >
            / event
          </span>
        )}
    ...
    <div className="flex-grow space-y-3 mb-8">
      {features.map((feature, idx) => (
        <div key={idx} className="flex items-start">
          <CheckCircle
            className={`w-5 h-5 mr-3 flex-shrink-0 ${recommended ? "text-gold-500" : "text-gold-600"}`}
          />
          <span
            className={`text-sm leading-relaxed ${recommended ? "text-gray-200" : "text-gray-600"}`}
          >
            {feature}
          </span>
        </div>
...
const QuizModal = ({ isOpen, onClose, onResult }) => {
  ...
  const questions = [
    {
      ...
      options: [
        {
          text: "I want to DIY it, but need a professional roadmap.",
          score: "essentials",
        },
        {
          text: "I'll handle the day-of, but need help finding vendors.",
          score: "signature",
        },
        {
          text: "I'm busy! I want someone to handle everything.",
          score: "elite",
        },
      ],
    },
    {
      ...
      options: [
        {
          text: "Not knowing where to start or how to style it.",
          score: "essentials",
        },
        {
          text: "Finding reliable vendors and managing the budget.",
          score: "signature",
        },
        {
          text: "Coordinating all the moving parts and design details.",
          score: "elite",
        },
      ],
    },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2D2424]/60 backdrop-blur-sm">
      <div className="bg-[#FFE4E6] rounded-2xl max-w-lg w-full p-8 shadow-2xl animate-fadeIn border border-white">
        {" "}
        {/* Updated BG */}
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-serif font-bold text-[#2D2424]">
            Let's find your match
          </h3>
          <button
            onClick={onClose}
            className="p-2 hover:bg-[#FECDD3] rounded-full text-[#2D2424]"
          >
            <X className="w-5 h-5" />
          </button>{" "}
          {/* Updated hover */}
        ...
        <div className="mb-8">
          <div className="flex space-x-2 mb-6">
            {questions.map((_, idx) => (
              <div
                key={idx}
                className={`h-1 flex-1 rounded-full ${idx <= step ? "bg-gold-500" : "bg-gray-300"}`}
              />
            ))}
          </div>
          <h4 className="text-xl font-medium mb-6 text-[#2D2424]">
...
export default function App() {
  ...
  const handleQuizResult = (result) => {
    ...
    if (element) {
      ...
      setTimeout(
        () =>
          alert(
            `Based on your answers, we recommend the ${result.toUpperCase()} package!`,
          ),
        500,
      );
    }
  ...
  return (
    <div className="min-h-screen bg-[#FFE4E6] font-sans text-[#2D2424] selection:bg-gold-200">
      {" "}
      {/* Updated Main BG to Bolder Blush */}
      {/* Navigation */}
      <nav
        className={`fixed w-full z-40 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm py-4" : "bg-transparent py-6"}`}
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            {!logoError ? (
              ...
              <div
                className={`font-serif text-2xl tracking-widest font-bold ${scrolled ? "text-[#2D2424]" : "text-[#2D2424] md:text-white"}`}
              >
          ...
          {/* Desktop Menu */}
          <div
            className={`hidden md:flex space-x-8 ${scrolled ? "text-gray-600" : "text-white"}`}
          >
            {["About", "Packages", "Process", "Contact", "Vendors"].map(
              (item) => (
                <button
                  key={item}
          ...
          {/* Mobile Toggle */}
          <button
            className={`md:hidden ${scrolled ? "text-[#2D2424]" : "text-[#2D2424]"}`}
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="absolute top-full left-0 w-full bg-[#FFE4E6] shadow-xl p-6 md:hidden flex flex-col space-y-4">
            {" "}
            {/* Updated BG */}
            {["About", "Packages", "Process", "Contact", "Vendors"].map(
              (item) => (
      ...
      <header className="relative h-screen flex items-center justify-center overflow-hidden bg-[#2D2424]">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center text-white">
          <div className="animate-fade-in-up">
            <h2 className="text-gold-300 font-medium tracking-[0.2em] uppercase mb-4 text-sm md:text-base">
              Event Consultation & Coordinating
            </h2>
            ...
            <p className="text-lg md:text-xl text-gray-100 max-w-2xl mx-auto mb-10 font-light">
              Are you overwhelmed with an upcoming event? Think of us as your
              middleman. You tell us your vision, and we source the vendors,
              design the details, and coordinate the chaos.
            </p>
      ...
      <section id="about" className="py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center gap-12">
            ...
            <div className="w-full md:w-1/2">
              <h3 className="text-gold-500 font-bold uppercase tracking-wider text-sm mb-2">
                Who We Are
              </h3>
              <h2 className="text-4xl font-serif font-bold mb-6 text-[#2D2424]">
                We Are Your Event Middleman
              </h2>
              <p className="text-gray-600 leading-relaxed mb-6">
                Luxe Vision is an event consultation and coordinating service
                covering everything from birthdays and weddings to corporate
                showers.
              </p>
              <p className="text-gray-600 leading-relaxed mb-8">
                We bridge the gap between your Pinterest board and the real
                world. Whether you need help sourcing vendors, creating mood
                boards, or designing digital invites, we've got you.
              </p>

              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-[#FFE4E6] rounded-lg border border-[#FECDD3]">
                  {" "}
                  {/* Updated BG & Border */}
                  <Users className="w-8 h-8 text-gold-500 mb-2" />
                  <h4 className="font-bold text-[#2D2424]">
                    Expert Vendor Sourcing
                  </h4>
                  <p className="text-xs text-gray-500">
                    We find the best teams for you.
                  </p>
                </div>
                <div className="p-4 bg-[#FFE4E6] rounded-lg border border-[#FECDD3]">
                  {" "}
      ...
      {/* Packages Section */}
      <section id="packages" className="py-24 bg-[#FFE4E6]">
        {" "}
        {/* Updated BG */}
        <div className="container mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-4xl font-serif font-bold mb-4 text-[#2D2424]">
              Curated Packages
            </h2>
            <p className="text-gray-600">
              Three levels of service to fit your budget and planning style.
          ...
          <div className="mt-12 max-w-3xl mx-auto bg-white rounded-xl p-8 shadow-md border border-[#FECDD3] text-center">
            {" "}
            {/* Updated Border */}
            ...
            <p className="text-gray-600 mb-2 leading-relaxed">
              We know every event is unique. Our packages are{" "}
              <strong>completely flexible</strong>—feel free to mix and match to
              get a little bit of all three!
            </p>
            <p className="text-gray-600 mb-4 leading-relaxed">
              <strong>On-site coordination</strong> is available as an add-on
              for <em>all</em> packages. Just ask us, and we'll make it work.
            </p>
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-500 italic">
              * Prices vary depending on location and market. 50% deposit
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center justify-between mb-16">
            ...
            <p className="text-gray-300 mt-4 md:mt-0 max-w-md text-right">
              From the first hello to the final timeline, we keep it organized
              and stress-free.
            </p>
          </div>
            ...
            {[
              {
                step: "01",
                title: "Consult",
                desc: "We meet virtually to discuss your theme, budget, and vision.",
              },
              {
                step: "02",
                title: "Proposal",
                desc: "You receive a custom proposal and contract.",
              },
              {
                step: "03",
                title: "Deposit",
                desc: "A 50% non-refundable deposit secures your date.",
              },
              {
                step: "04",
                title: "Planning",
                desc: "We create your moodboards, source vendors, and design assets.",
              },
              {
                step: "05",
                title: "Delivery",
                desc: "You receive your final Guide/Blueprint and execute a flawless event.",
              },
            ].map((item, index) => (
      ...
      <section id="contact" className="py-24 bg-white relative">
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-4xl mx-auto bg-[#FFE4E6] rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-[#FECDD3]">
            {" "}
            {/* Updated BG & Border */}
            {/* Contact Info */}
            <div className="bg-[#2D2424] text-white p-10 md:w-2/5 flex flex-col justify-between">
              <div>
                ...
                <p className="text-gray-300 mb-8 text-sm leading-relaxed">
                  Fill out the form and tell us a bit about your event. We'll
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <a
                      href="https://www.instagram.com/luxevision_events?igsh=MWlhamRiaTJqMGNvdw%3D%3D&utm_source=qr"
                      target="_blank"
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <a
                      href="https://www.facebook.com/share/1QhP7YuyJM/?mibextid=wwXIfr"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-gold-500 transition-colors"
                    >
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-400 mt-1">
                        If you want to contact us at non-office hours, leave us
                        a message on any of our socials.
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-10">
                ...
                <p className="text-xs text-gray-400">
                  Serving clients virtually & locally.
                </p>
              </div>
            </div>
            {/* Form */}
      ...
      {/* VENDOR SECTION - Added New Section */}
      <section id="vendors" className="py-24 bg-[#2D2424] text-white">
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-300 max-w-2xl mx-auto">
                Are you an event vendor and want to collaborate with us? Fill
                out your details below to join our exclusive vendor list, and we
      ...
      {/* Footer */}
      <footer className="bg-[#1a1515] text-white py-12 border-t border-gray-800">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center">
          <div className="mb-6 md:mb-0 text-center md:text-left">
            ...
            <p className="text-gray-400 text-sm">
              Turning your vision into reality, one detail at a time.
            </p>
          </div>
        ...
        <div className="container mx-auto px-6 mt-8 text-center text-xs text-gray-500">
          &copy; {new Date().getFullYear()} Luxe Vision Event Coordinating. All
          rights reserved.
        </div>
      </footer>
